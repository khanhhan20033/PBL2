...
void DP(int a[12][12],int  i,int j,bool visited[100][100],int row,int collumn)
{
	...
	{
	if (j-1>=0 && a[i][j-1]==0 && !visited[i][j-1]) DP(a,i,j-1,visited,row,collumn);
	if (j+1<collumn && a[i][j+1]==0 && !visited[i][j+1]) DP(a,i,j+1,visited,row,collumn);
	if (i+1<row && a[i+1][j]==0 && !visited[i+1][j]) DP(a,i+1,j,visited,row,collumn);}
}
void  DP1(int a[12][12],int i,int j,bool visited[100][100],int row,int collumn)
{
	...
	{
	if (j-1>=0 && a[i][j-1]==0 && !visited[i][j-1]) DP1(a,i,j-1,visited,row,collumn);
	if (j+1<collumn && a[i][j+1]==0 && !visited[i][j+1]) DP1(a,i,j+1,visited,row,collumn);
	if (i-1>row && a[i-1][j]==0 && !visited[i-1][j]) DP1(a,i-1,j,visited,row,collumn);}
}
...
{
	ifstream os("./suggest_sheets/data_film_sheet.txt");
	if (!os)
	{
